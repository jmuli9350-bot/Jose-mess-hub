...
function TeamPage({ members, onAdd, onDelete }: { members: Member[]; onAdd: () => void; onDelete: (id: number) => void }) {
  ...
  const saveEdit = (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!editing) return; const form = new FormData(event.currentTarget); const effectiveAt = new Date(`${String(form.get('effectiveDate') || today)}T${String(form.get('effectiveTime') || '00:00')}`).toISOString(); ;(window as any).__biztrackUpdateMember?.(editing.id, { name: String(form.get('name') || '').trim(), phone: String(form.get('phone') || '').trim(), email: String(form.get('email') || '').trim(), department: String(form.get('department') || 'Unassigned'), payRate: Number(form.get('payRate') || 0), payFrequency: String(form.get('payFrequency') || 'Daily') as PayFrequency, signInTime: String(form.get('signInTime') || editing.signInTime), signOutTime: String(form.get('signOutTime') || editing.signOutTime), initials: initialsFor(String(form.get('name') || editing.name)), effectiveAt }); setEditing(null) }
